...
const yogaTutorialPrice = 300;
const weightTrainingGroupPrice = 500;
const weightTraining1By1Price = 1500;
...
let AlexBudget = alexBudget - yogaTutorialPrice * 3 - weightTrainingGroupPrice - weightTraining1By1Price 
console.log(`Alex 買完課程了，他一共剩下 ${AlexBudget} 元`);

// ### 題目四：線稿圖截圖，看圖宣告變數
// 請參考資料夾內 q4.webp 圖片
// 請依照你看到的內容來嘗試設計變數和值（至少 3 個）
let coursePackageNum = 14;
let coursePackagePrice = 2520;
let coursePackageTime = 50;
let coursePackageAvgPrice = 180;
let coursePackageDiscount = 0.9;
...
let red = true;
let green = false;
let waitToGreenTime = 28;

// 4-2. 目前一起等待的機車有 8 台
let waitTogetherCycles = 8;
...
let b = 0; // 宣告一個 b 變數，並賦值 0 的數字型別
a = 13; // 以新的數字型別值 13 來重新賦值 a 變數，此時 console.log(a); 會印出 13
a = b + 4; // 以新的數字型別值 b+4 來重新賦值 a 變數，此時 console.log(a); 會印出運算結果 4
a - b; // 單純做數字型別變數的運算，並未賦值
b += 1; // 以新的數字型別值 b+1 來重新賦值 b 變數，此時 console.log(b); 會印出運算結果 1
...
let numberArr1 = [5, 10, 15]; // 宣告變數 numberArr1 為陣列型別並賦值 [5, 10, 15]
let numberArr2 = numberArr1; // 以 numberArr1 的位址來宣告變數 numberArr2 並賦值為 numberArr1 的值
numberArr2.push(20); // 新增元素 20 至 numberArr2 的尾端
numberArr2 = [25, 30, 35]; // 以新陣列 [25, 30, 35] 賦值 numberArr2
console.log(numberArr1, numberArr2); // 印出 numberArr1 與 numberArr2
